...
#define CONSTELLATIONSHIP_USE_HIP_NUMBER
...
#define RECSTR( w, x ) const char * w = #x; x
	...
	printf( "File read (%d constellation lines) (THIS OPERATION IS DEPRECATED)\n", lines );
	...
	for( i = 0; l = constellations[i]; i++ )
	{
		...
		{
#ifdef CONSTELLATIONSHIP_USE_HIP_NUMBER
			linesegs[curlineseg] = atoi( fields[2+k] );
#else
